...
pub(crate) fn get_count_by_year_internal(state: &AppState) -> Vec<(NaiveDate, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_count_by_date()
}
...
pub(crate) fn get_mag_depth_pairs_internal(state: &AppState) -> Vec<(f64, f64)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_mag_depth_pairs()
}
...
pub(crate) fn get_hourly_frequency_internal(state: &AppState) -> Vec<(u32, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_hourly_frequency()
}

/// Get monthly frequency distribution
pub(crate) fn get_monthly_frequency_internal(state: &AppState) -> Vec<(u32, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_monthly_frequency()
}

/// Get geographic hotspots by region
pub(crate) fn get_region_hotspots_internal(state: &AppState) -> Vec<(String, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_region_hotspots()
}

/// Get coordinate clusters for mapping
pub(crate) fn get_coordinate_clusters_internal(state: &AppState) -> Vec<(f64, f64, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_coordinate_clusters()
}

/// Get Gutenberg-Richter b-value
pub(crate) fn get_b_value_internal(state: &AppState) -> f64 {
    let state = state.lock().unwrap();
    state.get_analytics().get_b_value()
}

/// Get magnitude-frequency relationship data
pub(crate) fn get_magnitude_frequency_data_internal(state: &AppState) -> Vec<(f64, u32, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_magnitude_frequency_data()
}

/// Get risk assessment metrics
pub(crate) fn get_risk_metrics_internal(state: &AppState) -> (f64, f64, f64, f64) {
    let state = state.lock().unwrap();
    state.get_analytics().get_risk_metrics()
}

/// Get total seismic energy released
pub(crate) fn get_total_energy_internal(state: &AppState) -> f64 {
    let state = state.lock().unwrap();
    state.get_analytics().get_total_energy()
}

/// Get weekly frequency distribution with weekday names
pub(crate) fn get_weekly_frequency_internal(state: &AppState) -> Vec<(String, u32)> {
    let state = state.lock().unwrap();
    state.get_analytics().get_weekly_frequency()
}
